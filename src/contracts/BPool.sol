...
contract BPool is BToken, BMath, IBPool {
  ...
  function _afterFinalize() internal virtual {
    // NOTE: Adds ' (token1-token2-...)' to the ERC20 symbol
    _symbol = string(abi.encodePacked(symbol(), ' ('));
    for (uint256 i; i < _tokens.length; i++) {
      string memory tokenSymbol = IERC20Metadata(_tokens[i]).symbol();
      _symbol = string(abi.encodePacked(_symbol, tokenSymbol));
      if (i != _tokens.length - 1) {
        _symbol = string(abi.encodePacked(_symbol, '-'));
      }
    }
    _symbol = string(abi.encodePacked(_symbol, ')'));
  }
