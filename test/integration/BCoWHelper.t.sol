...
contract ConstantProductHelperForkedTest is Test {
  ...
  function testValidOrder(uint256 balanceToken0, uint256 balanceToken1, uint256 priceSkewness) public {
    IBCoWPool pool = IBCoWPool(address(basicPool));
    balanceToken0 = bound(balanceToken0, 1e18, 1_000_000_000_000e18);
    balanceToken1 = bound(balanceToken1, 1e18, 1_000_000_000_000e18);
    // skew the price by max 50% (more could result in reverts bc of max swap ratio)
    priceSkewness = bound(priceSkewness, 50, 99);
    ...
    prices[1] = balanceToken0 * priceSkewness / 100;
  ...
  // NOTE: this test checks that the order generated by the helper is valid
  function testValidOrderInverse(uint256 balanceToken0, uint256 balanceToken1, uint256 priceSkewness) public {
    IBCoWPool pool = IBCoWPool(address(basicPool));
    balanceToken0 = bound(balanceToken0, 1e18, 1_000_000_000_000e18);
    balanceToken1 = bound(balanceToken1, 1e18, 1_000_000_000_000e18);
    // skew the price by max 50% (more could result in reverts bc of max swap ratio)
    ...
    uint256[] memory prices = new uint256[](2);
    prices[0] = balanceToken1 * priceSkewness / 100;
    prices[1] = balanceToken0;
